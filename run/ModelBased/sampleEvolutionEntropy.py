...
#         ent = bww.sampleE[0,indice[i][j]][bww.sampleE[0,indice[i][j]] <> 0]
#         ind = indice[i]
#         #----------------------------------
#         # Plot
#         #----------------------------------
...
for j in xrange(0, 3):
    #ent = bww.sampleE[0,indice[i][j]][bww.sampleE[0,indice[i][j]] <> 0]
    ent = bww.sampleE[0,indice[i][j]][0:bad[j]]
    ind = indice[i]
    #----------------------------------
    # Plot
    #----------------------------------
    subplot(1,3,j+1)
    plot(range(0, len(ent)), ent, 'o-', markersize = 1.2, linewidth=2.4)
    xlabel("Nb inferences", fontsize = 15)
    ylabel("Entropy", fontsize = 15)
    legend()
    #grid()
    ylim(0,2.5)
    xlim(0,10)
    ...
    for k,l in zip(range(0,45,3)[1:j+1], r[0:len(range(0,45,3)[1:j+1])][::-1]):
        if l == 0:
            axvline(k, 0, 1, linestyle = '--', color = 'red', linewidth=1.5)
        elif l == 1:
            axvline(k, 0, 1, linestyle = '--', color = 'green', linewidth = 1.5)
subplots_adjust(wspace = 0.5)            
savefig('../../../Dropbox/ISIR/JournalClub/images/fig_entropy_stim1.pdf', bbox_inches = 'tight')
show()        
