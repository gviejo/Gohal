...
def testModel():
    ...
    for i in xrange(nb_blocs):
        ...
        for j in xrange(nb_trials):
            ...
            qlearning.updateValue(reward)
    qlearning.state = convertStimulus(np.array(qlearning.state))
    qlearning.action = convertAction(np.array(qlearning.action))
    qlearning.responses = np.array(qlearning.responses)
    qlearning.reaction = np.array(qlearning.reaction)
...
qlearning = QLearning('k', cats.states, cats.actions, gamma, alpha, beta)
...
#order data
# -----------------------------------
pcr = extractStimulusPresentation(qlearning.responses, qlearning.state, qlearning.action, qlearning.responses)
pcr_human = extractStimulusPresentation(human.responses['meg'], human.stimulus['meg'], human.action['meg'], human.responses['meg'])

qlearning.reaction = computeEntropy(np.ones(5)*0.2, beta) - qlearning.reaction
entropy = extractStimulusPresentation(qlearning.reaction, qlearning.state, qlearning.action, qlearning.responses)
